...
export SRC=$PWD/num-integer
export WORKING_DIR=$PWD/../src

# install syn package
git clone https://github.com/rust-num/num-integer.git
cd $SRC
git reset --hard 80c2b227909b69cdb0bbe1331ac3f167002539a6

# build bench tests without running it (make sure that you set the environmetal settings by ERASan/env/env.sh)
RUSTFLAGS="--emit=llvm-ir" cargo bench --bench average --no-run
