...
const std::string ExtractValVFGNode::toString() const {
    std::string str;
    raw_string_ostream rawstr(str);
    rawstr << "ExtractValVFGNode ID: " << getId() << " ";
    rawstr << getPAGEdge()->toString();
    return rawstr.str();
}

const std::string InsertValVFGNode::toString() const {
    std::string str;
    raw_string_ostream rawstr(str);
    rawstr << "InsertValVFGNode ID: " << getId() << " ";
    rawstr << "PAGNode: " <<res->getId()<<"\t";
    rawstr << res->toString();
    return rawstr.str();
}
...
void VFG::addVFGNodes()
{
                ...
                copys.end(); iter != eiter; ++iter)
    {
        ...
        if(!isPhiCopyEdge(edge) && !isInsertValEdge(edge))
            addCopyVFGNode(edge);
    }
    ...
    // initialize extractVal nodes
    PAGEdge::PAGEdgeSetTy& extractVals = getPAGEdgeSet(PAGEdge::ExtractVal);
    for(PAGEdge::PAGEdgeSetTy::iterator iter = extractVals.begin(), eiter = extractVals.end(); iter != eiter; ++iter){
        addExtractValVFGNode(SVFUtil::cast<ExtractValPE>(*iter));
    }

    // initialize insertVal nodes
    PAGEdge::PAGEdgeSetTy& insertVals = getPAGEdgeSet(PAGEdge::InsertVal);
    for(PAGEdge::PAGEdgeSetTy::iterator iter = insertVals.begin(), eiter = insertVals.end(); iter != eiter; ++iter){
        addInsertValVFGNode(SVFUtil::cast<InsertValPE>(*iter));
    }
...
void VFG::connectDirectVFGEdges()
{
    ...
    for(iterator it = begin(), eit = end(); it!=eit; ++it)
    {
        ...
        }else if(InsertValVFGNode* ivNode = SVFUtil::dyn_cast<InsertValVFGNode>(node)){
            addIntraDirectVFEdge(getDef(ivNode->getParent()), nodeId);
            if(ivNode->getInserted()->hasValue())
                addIntraDirectVFEdge(getDef(ivNode->getInserted()), nodeId);
        }
...
const PAGNode* VFG::getLHSTopLevPtr(const VFGNode* node) const
{
    ...
    else if(const InsertValVFGNode* ivNode = SVFUtil::dyn_cast<InsertValVFGNode>(node))
        return ivNode->getRes();
    else if(const ExtractValVFGNode* evNode = SVFUtil::dyn_cast<ExtractValVFGNode>(node))
        return evNode->getPAGDstNode();
    else
