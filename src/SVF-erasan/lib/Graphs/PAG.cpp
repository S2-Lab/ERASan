...
const std::string ExtractValPE::toString() const {
    std::string str;
    raw_string_ostream rawstr(str);
    rawstr << "ExtractValPE: [" << getDstID() << "<--" << getSrcID() << "]\t";
    if (Options::PAGDotGraphShorter) {
        rawstr << "\n";
    }
    rawstr << value2String(getValue());
...
const std::string InsertValPE::toString() const {
    std::string str;
    raw_string_ostream rawstr(str);
    rawstr << "InsertValPE: [" << getDstID() << "<--" << getSrcID() << "]\t";
    if (Options::PAGDotGraphShorter) {
        rawstr << "\n";
    }
    rawstr << value2String(getValue());
 ...
 * Add ExtractVal edge
 */
ExtractValPE* PAG::addExtractValPE(NodeID src, NodeID dst, const LocationSet &ls)
{

    PAGNode* srcNode = getPAGNode(src);
    PAGNode* dstNode = getPAGNode(dst);
    ExtractValPE* edge = new ExtractValPE(srcNode,dstNode,ls);
    addEdge(srcNode,dstNode,edge);
    return edge;
}
 ...
 * Add ExtractVal edge
 */
InsertValPE* PAG::addInsertValPE(NodeID src, NodeID dst, const LocationSet &ls)
{

    PAGNode* srcNode = getPAGNode(src);
    PAGNode* dstNode = getPAGNode(dst);
    InsertValPE* edge = new InsertValPE(srcNode,dstNode,ls);
    addEdge(srcNode,dstNode,edge);
    return edge;
}
...
const std::set<std::pair<const LocationSet,NodeID>> PAG::getInsertValObjNodes(NodeID id){
    auto it = insertValPtsMap.find(id);
    if(it == insertValPtsMap.end()) {
        std::set<std::pair<const LocationSet, NodeID>> ts;
        insertValPtsMap.insert(std::make_pair(id,ts));
        return ts;
    }
    assert(it != insertValPtsMap.end() && "corresponding insertVal node info not found");
    return it->second;
}

bool PAG::getExtractValNodes(NodeID id, const LocationSet& ls){
    std::pair<const LocationSet, NodeID> pp = std::make_pair(ls,id);
    std::set<NodeID> ret;
    for(auto it: insertValPtsMap){
...
NodeID PAG::getInsertValObjNode(NodeID id, const LocationSet &ls) {
    auto it = insertValPtsMap.find(id);
    assert(it != insertValPtsMap.end() && "corresponding insertVal node info not found");
    for(auto pair: it->second){
        if(pair.first == ls)
    ...
    assert(false && "corresponding locationset info not found");
    return -1;
}

NodeID PAG::addInsertValObjNode(NodeID id, const LocationSet &ls, NodeID objID) {
    auto it = insertValPtsMap.find(id);
    if(it == insertValPtsMap.end()){
        std::set<std::pair<const LocationSet, NodeID>> ts;
        ts.insert(std::make_pair(ls,objID));
        insertValPtsMap.insert(std::make_pair(id,ts));
    }else {
        std::pair<const LocationSet, NodeID> pp = std::make_pair(ls,objID);
        it->second.insert(pp);
    }
