...
InsertValCGEdge* ConstraintGraph::addInsertValCGEdge(NodeID src, NodeID dst, const LocationSet& ls) {
    ConstraintNode* srcNode = getConstraintNode(src);
    ConstraintNode* dstNode = getConstraintNode(dst);
    if(hasEdge(srcNode, dstNode, ConstraintEdge::InsertVal))
        return nullptr;

    InsertValCGEdge* edge = new InsertValCGEdge(srcNode, dstNode, ls, edgeIndex++);
    bool added = InsertValCGEdgeSet.insert(edge).second;
    assert(added && "not added?");
    srcNode->addOutgoingInsertValEdge(edge);
    dstNode->addIncomingInsertValEdge(edge);
    return edge;
}

ExtractValCGEdge* ConstraintGraph::addExtractValCGEdge(NodeID src, NodeID dst, const LocationSet &ls) {
    ConstraintNode* srcNode = getConstraintNode(src);
    ConstraintNode* dstNode = getConstraintNode(dst);
    if(hasEdge(srcNode, dstNode, ConstraintEdge::ExtractVal))
        return nullptr;

    ExtractValCGEdge* edge = new ExtractValCGEdge(srcNode, dstNode, ls, edgeIndex++);
    bool added = ExtractValCGEdgeSet.insert(edge).second;
    assert(added && "not added?");
    srcNode->addOutgoingExtractValEdge(edge);
    dstNode->addIncomingExtractValEdge(edge);
    return edge;
}
