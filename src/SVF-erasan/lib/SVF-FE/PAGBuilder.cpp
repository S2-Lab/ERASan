...
/*
 * Compute the object node offset according to Extract/Insert instruction
 */
bool PAGBuilder::computeExtInsOffest(const User *V, LocationSet &ls) {
    return SymbolTableInfo::SymbolInfo()->computeGepOffset(V, ls);
}
/*!
...
// /* Handling ExtractValue Instruction which is heavily used in Rust */
// void PAGBuilder::visitExtractValueInst(ExtractValueInst  &inst)
// {
//     NodeID dst = getValueNode(&inst);
//     /* Handling ExtractValue Instruction which is heavily used in Rust */
//     NodeID src = getValueNode(inst.getAggregateOperand());
...
//     NodeID src = getValueNode(inst.getAggregateOperand());
//     LocationSet ls;
//     computeGepOffset(&inst, ls);
//     addExtractValPE(src,dst,ls);
// }

// /* Handling InsertValue Instruction which is heavily used in Rust */
// void PAGBuilder::visitInsertValueInst(InsertValueInst  &inst)
// {

//     DBOUT(DPAGBuild, outs() << "process InsertValue  " << inst << " \n");

//     NodeID src = getValueNode(inst.getInsertedValueOperand());

//     NodeID dst = getValueNode(&inst);
    
//     NodeID aggregate = getValueNode(inst.getAggregateOperand());
//     if(!SVF::SVFUtil::isa<UndefValue>(inst.getAggregateOperand())) {
//         addCopyEdge(aggregate, dst);
//     }
//     else {
//         addCopyEdge(src, dst);
//     }
...
//     // LocationSet ls;
//     // bool constGep = computeGepOffset(&inst, ls);
//     // addGepEdge(aggregate, dst, ls, constGep);
...
// {
//     NodeID dst = getValueNode(&inst);
//     Type* insertedType = inst.getInsertedValueOperand()->getType();
//     NodeID src;
//     if(insertedType->isPointerTy() || (insertedType->isAggregateType() && SymbolTableInfo::SymbolInfo()->enclosesPointer(insertedType))) {
//         src = getValueNode(inst.getInsertedValueOperand()->stripPointerCasts());
//     }else
//         src = pag->getBlackHoleNode();

//     //errs()<<"Aggr: "<<*inst.getAggregateOperand()<<": is constant: "<<isConstantData(inst.getAggregateOperand())<<"\n";
//     NodeID im = isConstantData(inst.getAggregateOperand())? pag->getNullPtr(): getValueNode(inst.getAggregateOperand());
//     LocationSet ls;
//     computeGepOffset(&inst, ls);
//     const CopyPE* cpy = addCopyEdge(im,dst);
//     const InsertValPE* ivEdge = addInsertValPE(src,dst,ls);
//     pag->addInsertValNode(pag->getPAGNode(dst),cpy,ivEdge);
// }
