...
private:
    ...
    ConstraintEdge::ConstraintEdgeSetTy insertValInEdges;
    ConstraintEdge::ConstraintEdgeSetTy insertValOutEdges;

    ConstraintEdge::ConstraintEdgeSetTy extractValInEdges;
    ConstraintEdge::ConstraintEdgeSetTy extractValOutEdges;
...
public:
    ...
    {
        return (kind == ConstraintEdge::Load || kind == ConstraintEdge::Store || kind == ConstraintEdge::InsertVal || kind == ConstraintEdge::ExtractVal);
    }
    ...
    inline const ConstraintEdge::ConstraintEdgeSetTy& getExtractValInEdges() const
    {
        return extractValInEdges;
    }

    inline const ConstraintEdge::ConstraintEdgeSetTy& getExtractValOutEdges() const
    {
        return extractValOutEdges;
    }

    inline const ConstraintEdge::ConstraintEdgeSetTy& getInsertValInEdges() const
    {
        return insertValInEdges;
    }

    inline const ConstraintEdge::ConstraintEdgeSetTy& getInsertValOutEdges() const
    {
        return insertValOutEdges;
    }
    //@}
    ...
    inline void addIncomingInsertValEdge(InsertValCGEdge* ivCEEdge){
        insertValInEdges.insert(ivCEEdge);
        addIncomingEdge(ivCEEdge);
    }

    inline void addOutgoingInsertValEdge(InsertValCGEdge* ivCEEdge){
        bool added1 = insertValOutEdges.insert(ivCEEdge).second;
        bool added2 = addOutgoingEdge(ivCEEdge);
        assert(added1 && added2 && "edge not added, duplicated adding!!");
    }

    inline void addIncomingExtractValEdge(ExtractValCGEdge* evCEEdge){
        extractValInEdges.insert(evCEEdge);
        addIncomingEdge(evCEEdge);
    }

    inline void addOutgoingExtractValEdge(ExtractValCGEdge* evCEEdge){
        bool added1 = extractValOutEdges.insert(evCEEdge).second;
        bool added2 = addOutgoingEdge(evCEEdge);
        assert(added1 && added2 && "edge not added, duplicated adding!!");
    }
    //@}
    ...
    inline void removeIncomingExtractValEdge(ExtractValCGEdge* inEdge){
        Size_t num1 = extractValInEdges.erase(inEdge);
        Size_t num2 = removeIncomingEdge(inEdge);
        assert((num1 && num2) && "edge not in the set, can not remove!!!");
    }
    inline void removeIncomingInsertValEdge(ExtractValCGEdge* inEdge){
        Size_t num1 = insertValInEdges.erase(inEdge);
        Size_t num2 = removeIncomingEdge(inEdge);
        assert((num1 && num2) && "edge not in the set, can not remove!!!");
    }
    inline void removeOutgoingInsertValEdge(ExtractValCGEdge* outEdge){
        Size_t num1 = insertValOutEdges.erase(outEdge);
        Size_t num2 = removeOutgoingEdge(outEdge);
        assert((num1 && num2) && "edge not in the set, can not remove!!!");
    }
    inline void removeOutgoingExtractValEdge(ExtractValCGEdge* outEdge){
        Size_t num1 = extractValOutEdges.erase(outEdge);
        Size_t num2 = removeOutgoingEdge(outEdge);
        assert((num1 && num2) && "edge not in the set, can not remove!!!");
    }
    //@}
