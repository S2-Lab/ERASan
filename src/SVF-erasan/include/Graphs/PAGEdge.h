...
public:
    ...
    enum PEDGEK
    {
        Addr, Copy, Store, Load, Call, Ret, NormalGep, VariantGep, ThreadFork, ThreadJoin, Cmp, BinaryOp, UnaryOp, ExtractVal, InsertVal,
    };
...
public:
    ...
    static inline bool classof(const GenericPAGEdgeTy *edge)
    {
        return edge->getEdgeKind() == PAGEdge::Addr ||
               ...
               edge->getEdgeKind() == PAGEdge::ExtractVal  ||
               edge->getEdgeKind() == PAGEdge::InsertVal;
    }
    ///@}
 ...
 * ExtractValPE
 */
class ExtractValPE: public PAGEdge
{
private:
    ExtractValPE();                      ///< place holder
    ExtractValPE(const ExtractValPE &);  ///< place holder
    void operator=(const ExtractValPE &); ///< place holder
    LocationSet ls;
public:
    /// Methods for support type inquiry through isa, cast, and dyn_cast:
    //@{
    static inline bool classof(const ExtractValPE *)
    {
        return true;
    }
    static inline bool classof(const PAGEdge *edge)
    {
        return 	edge->getEdgeKind() == PAGEdge::ExtractVal;
    }
    static inline bool classof(const GenericPAGEdgeTy *edge)
    {
        return	edge->getEdgeKind() == PAGEdge::ExtractVal;
    ...
    //@}
    /// constructor
    ExtractValPE(PAGNode* s, PAGNode* d, const LocationSet& l) : PAGEdge(s,d,PAGEdge::ExtractVal), ls(l)
    {

    }

    virtual const std::string toString() const;
    /// offset of the ExtractVal edge
 ...
 * InsertValPE
 */
class InsertValPE: public PAGEdge
{
private:
    InsertValPE();                      ///< place holder
    InsertValPE(const InsertValPE &);  ///< place holder
    void operator=(const InsertValPE &); ///< place holder
    LocationSet ls;
public:
    /// Methods for support type inquiry through isa, cast, and dyn_cast:
    //@{
    static inline bool classof(const InsertValPE *)
    {
        return true;
    }
    static inline bool classof(const PAGEdge *edge)
    {
        return 	edge->getEdgeKind() == PAGEdge::InsertVal;
    }
    static inline bool classof(const GenericPAGEdgeTy *edge)
    {
        return	edge->getEdgeKind() == PAGEdge::InsertVal;
    ...
    //@}
    /// constructor
    InsertValPE(PAGNode* s, PAGNode* d, const LocationSet& l) : PAGEdge(s,d,PAGEdge::InsertVal), ls(l)
    {

    }

    virtual const std::string toString() const;
    /// offset of the InsertVal edge
    inline u32_t getOffset() const
    {
        return ls.getOffset();
    }
