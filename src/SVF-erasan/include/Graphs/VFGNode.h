...
public:
    ...
    enum VFGNodeK
    {
        ...
        FunRet, APIN, APOUT, FPIN, FPOUT, NPtr, DummyVProp, InsertVal, ExtractVal
    };
...
/*!
 * VFGNode for ExtractVal
 */
class ExtractValVFGNode: public StmtVFGNode
{
private:
    ExtractValVFGNode();                      ///< place holder
    ExtractValVFGNode(const ExtractValVFGNode &);  ///< place holder
    void operator=(const ExtractValVFGNode &); ///< place holder
public:
    /// Constructor
    ExtractValVFGNode(NodeID id,const ExtractValPE* edge): StmtVFGNode(id,edge,ExtractVal)
    {

    }
    /// Methods for support type inquiry through isa, cast, and dyn_cast:
    //@{
    static inline bool classof(const ExtractValVFGNode *)
    {
        return true;
    }
    static inline bool classof(const StmtVFGNode *node)
    {
        return node->getNodeKind() == ExtractVal;
    }
    static inline bool classof(const VFGNode *node)
    {
        return node->getNodeKind() == ExtractVal;
...
/*!
 * VFGNode for InsertVal
 */
class InsertValVFGNode: public VFGNode
{

private:
    InsertValVFGNode();                      ///< place holder
    InsertValVFGNode(const InsertValVFGNode &);  ///< place holder
    void operator=(const InsertValVFGNode &); ///< place holder
protected:
    const PAGNode* res;
    const PAGNode* inserted;
    const PAGNode* parent;
    const LocationSet ls;
public:
    /// Constructor
    InsertValVFGNode(NodeID id,const PAGNode* r, const LocationSet& l): VFGNode(id,InsertVal), res(r), ls(l)
    {

    }
    /// Methods for support type inquiry through isa, cast, and dyn_cast:
    //@{
    static inline bool classof(const InsertValVFGNode *)
    {
        return true;
    }
    static inline bool classof(const VFGNode *node)
    {
        return node->getNodeKind() == InsertVal;
    }
    static inline bool classof(const GenericVFGNodeTy *node)
    {
        return node->getNodeKind() == InsertVal;
    ...
    inline const PAGNode* getInserted() const{
        return inserted;
    }

    inline const PAGNode* getParent() const{
        return parent;
    }
    ...
    inline void setInserted(const PAGNode* in){
        inserted = in;
    }

    inline void setParent(const PAGNode* par){
        parent = par;
    }
