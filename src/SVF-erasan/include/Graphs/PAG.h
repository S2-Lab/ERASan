...
public:
    ...
    typedef Map<const PAGNode*, std::pair<const InsertValPE*, const CopyPE*>> InsertValMap;
    typedef Map<NodeID, std::set<std::pair<const LocationSet,NodeID>>> InsertValPtsMap;
...
private:
    ...
    InsertValMap insertValMap;
    InsertValPtsMap insertValPtsMap;
...
public:
    ...
    inline InsertValMap& getInsertValCopyMap(){
        return insertValMap;
    }
    ...
    inline void addInsertValNode(const PAGNode* dest, const CopyPE* cpyEdge, const InsertValPE* iEdge){
        insertValMap.insert(std::make_pair(dest,std::make_pair(iEdge,cpyEdge)));
    }

    inline bool isInsertValNode(const PAGNode* node) const{
        return insertValMap.find(node) != insertValMap.end();
    }

    inline std::pair<const InsertValPE*,const CopyPE*>& getInsertValEdges(const PAGNode* node){
        InsertValMap::iterator it = insertValMap.find(node);
        assert(it != insertValMap.end() && "InsertValCopyPE not found");
        return it->second;
    }
    ...
    const std::set<std::pair<const LocationSet,NodeID>> getInsertValObjNodes(NodeID id);
    bool getExtractValNodes(NodeID id, const LocationSet& ls);
    NodeID getInsertValObjNode(NodeID id, const LocationSet& ls);
    NodeID addInsertValObjNode(NodeID id, const LocationSet& ls, NodeID objID);
    ...
    /// Add ExtractVal edge
    ExtractValPE* addExtractValPE(NodeID src, NodeID dst, const LocationSet& ls);
    /// Add InsertVal edge
    InsertValPE* addInsertValPE(NodeID src, NodeID dst, const LocationSet& ls);
    /// Add Offset(Gep) edge
