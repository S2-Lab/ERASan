...
//Loggging
static uint64_t TotalInstruction = 0;
static uint64_t TotalInstrument = 0;
static uint64_t RawptrInstrument = 0;
static uint64_t RawptrDerefInstrument = 0;
static uint64_t HeapafterDropAccessInstrument = 0;
static uint64_t StackafterReturnAccessInstrument = 0;
static uint64_t DebloatingInstrument = 0;
static uint64_t NonDebloatingInstrument = 0;
static uint64_t OnlyERASanDelete = 0;
static uint64_t OnlyDebloatingASanDelete = 0;
static uint64_t CombinationERASanDebloatingASanDelete = 0;
static uint64_t CombinationERASanDebloatingASanShouldCheck = 0;
...
static cl::opt<bool>
    ClUseStackSafety("asan-use-stack-safety", cl::Hidden, cl::init(false),
                     ...
                     cl::Optional); //ASAN--: Removing Unsatisfiable Checks
...
struct AddressSanitizer {
  ...
  bool instrumentFunction(Function &F, const TargetLibraryInfo *TLI, AliasAnalysis *AA, LoopInfo *LI, ScalarEvolution *SE); //ASan-- instrumentFunction
...
PreservedAnalyses ModuleAddressSanitizerPass::run(Module &M,
  ...
  /* Set SanitizerAddress Attribute */                                                 
  for (Function &F : M) {
  ...
  for (Function &F : M) {
    ...
    LoopInfo *LI = nullptr;
    ScalarEvolution *SE = nullptr;
    Modified |= FunctionSanitizer.instrumentFunction(F, &TLI, AA, LI, SE);
  }
...
void AddressSanitizer::instrumentMop(ObjectSizeOffsetVisitor &ObjSizeVis,
  ...
  if (O.MaybeMask) {
    instrumentMaskedLoadOrStore(this, DL, IntptrTy, O.MaybeMask, O.getInsn(),
                                Addr, O.Alignment, Granularity, O.OpType, //Poriting Point
...
void AddressSanitizer::slimAsanOptimization(Function &F, SmallVector<InterestingMemoryOperand, 16> &OperandsToInstrument, AliasAnalysis *AA, LoopInfo *LI, ScalarEvolution *SE, ObjectSizeOffsetVisitor &ObjSizeVis, bool UseCalls) {
    ...
    if (AA && !OperandsToInstrument.empty()) {
...
bool AddressSanitizer::instrumentFunction(Function &F,
  ...
  // ASan-- : Keep Original Instruction
  SmallVector<InterestingMemoryOperand, 16> OriginOperandsToInstrument(OperandsToInstrument);
  // ASAN-- : Optimizations
  slimAsanOptimization(F, OperandsToInstrument, AA, LI, SE, ObjSizeVis, UseCalls);

  // ASan-- : Add Metadata to debloateing instruction
  for (auto &Oper : OperandsToInstrument) {
    Instruction *Insn = Oper.getInsn();
    LLVMContext& C = Insn->getContext();
    MDNode* N = MDNode::get(C, MDString::get(C, "my md string content"));
    Insn->setMetadata("debloating", N);
  }

  //Count Total Instrumentation
  for (auto &Operand : OriginOperandsToInstrument) {
    TotalInstrument++;
  }

  //Count ASan--
  for (auto &Operand : OriginOperandsToInstrument) {
    if(Operand.getInsn()->hasMetadata("debloating")){
      DebloatingInstrument++;
    }
  }

  //Count ERASan
  for (auto &Operand : OriginOperandsToInstrument) {
    if(Operand.getInsn()->hasMetadata("rawptr")){
      RawptrInstrument++;
    }
    else if(Operand.getInsn()->hasMetadata("rawptr_deref")){
      RawptrDerefInstrument++;
    }
    else if (Operand.getInsn()->hasMetadata("HeapafterDropAccess")){
      HeapafterDropAccessInstrument++;
    }
    else if (Operand.getInsn()->hasMetadata("StackafterReturnAccess")){
      StackafterReturnAccessInstrument++;
    }
  }

  //Count ERASan and ASan-- Combination
  for (auto &Operand : OriginOperandsToInstrument) {
    if((!Operand.getInsn()->hasMetadata("rawptr") && !Operand.getInsn()->hasMetadata("rawptr_deref") && !Operand.getInsn()->hasMetadata("HeapafterDropAccess") && !Operand.getInsn()->hasMetadata("StackafterReturnAccess")) && !Operand.getInsn()->hasMetadata("debloating")){
      CombinationERASanDebloatingASanDelete++;
    }
  }
  ...
  for (auto &Operand : OriginOperandsToInstrument) {
    if((Operand.getInsn()->hasMetadata("rawptr") || Operand.getInsn()->hasMetadata("rawptr_deref") || Operand.getInsn()->hasMetadata("HeapafterDropAccess") || Operand.getInsn()->hasMetadata("StackafterReturnAccess")) && !Operand.getInsn()->hasMetadata("debloating")){
      OnlyDebloatingASanDelete++;
    }
  }

  for (auto &Operand : OriginOperandsToInstrument) {
    if((Operand.getInsn()->hasMetadata("rawptr") || Operand.getInsn()->hasMetadata("rawptr_deref") || Operand.getInsn()->hasMetadata("HeapafterDropAccess") || Operand.getInsn()->hasMetadata("StackafterReturnAccess")) && Operand.getInsn()->hasMetadata("debloating")){
      CombinationERASanDebloatingASanShouldCheck++;
    }
  }
  ...
  for (auto &Operand : OriginOperandsToInstrument) {
    if (!suppressInstrumentationSiteForDebug(NumInstrumented)){
        instrumentMop(ObjSizeVis, Operand, UseCalls,F.getParent()->getDataLayout());
    }
    FunctionModified = true;
  }
