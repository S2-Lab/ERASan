...
impl<'a, 'll, 'tcx> BuilderMethods<'a, 'tcx> for Builder<'a, 'll, 'tcx> {
    ...
    /* Register Metadata Kind */
    fn register_rawptr_metadata_kind(&self) -> u32 {
        let name = "rawptr";
        unsafe {
    ...
    /* Set Metadata for Raw Pointer */
    fn rawptr_metadata(&mut self, value: &'ll Value) {
        let kind = self.register_rawptr_metadata_kind();
        let data = "rawptr";
        let data_val = unsafe {
    ...
    /* Register Metadata Kind */
    fn register_rawptrderef_metadata_kind(&self) -> u32 {
        let name = "rawptr_deref";
        unsafe {
            llvm::LLVMGetMDKindIDInContext(self.cx.llcx, name.as_ptr() as *const _, name.len() as u32)
        }
    }
    ...
    /* Set Metadata for Raw Pointer */
    fn rawptrderef_metadata(&mut self, value: &'ll Value) {
        let kind = self.register_rawptrderef_metadata_kind();
        let data = "rawptr_deref";
        let data_val = unsafe {
            llvm::LLVMMDStringInContext(self.cx.llcx, data.as_ptr() as *const _, data.len() as u32)
        };
        let metadata_node = unsafe {
    ...
    // /* Register Metadata Kind */
    fn register_unsafe_metadata_kind(&self) -> u32 {
        let name = "unsafe";
        unsafe {
            llvm::LLVMGetMDKindIDInContext(self.cx.llcx, name.as_ptr() as *const _, name.len() as u32)
        }
    ...
    /* Set Metadata for Raw Pointer */
    fn unsafe_metadata(&mut self, value: &'ll Value) {
        let kind: u32 = self.register_unsafe_metadata_kind();
        let data = "unsafe";
        let data_val = unsafe {
            llvm::LLVMMDStringInContext(self.cx.llcx, data.as_ptr() as *const _, data.len() as u32)
        };
    ...
    fn inbounds_gep(
        ...
        unsafe {
            let instr = llvm::LLVMBuildInBoundsGEP2(
                self.llbuilder,
                ty,
                ptr,
                indices.as_ptr(),
    ...
    fn struct_gep(&mut self, ty: &'ll Type, ptr: &'ll Value, idx: u64) -> &'ll Value {
        ...
        unsafe { 
            let instr = llvm::LLVMBuildStructGEP2(self.llbuilder, ty, ptr, idx as c_uint, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }   
            instr
    ...
    fn trunc(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe { 
            let instr = llvm::LLVMBuildTrunc(self.llbuilder, val, dest_ty, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    fn sext(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe { 
            let instr = llvm::LLVMBuildSExt(self.llbuilder, val, dest_ty, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    fn fptoui(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        ...
        unsafe { 
            let instr = llvm::LLVMBuildFPToUI(self.llbuilder, val, dest_ty, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    fn fptosi(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        ...
        unsafe { 
            let instr = llvm::LLVMBuildFPToSI(self.llbuilder, val, dest_ty, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    fn uitofp(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe { 
            let instr = llvm::LLVMBuildUIToFP(self.llbuilder, val, dest_ty, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    fn sitofp(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe {
            let instr = llvm::LLVMBuildSIToFP(self.llbuilder, val, dest_ty, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    fn fptrunc(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe { 
            let instr = llvm::LLVMBuildFPTrunc(self.llbuilder, val, dest_ty, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    fn fpext(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe { 
            let instr = llvm::LLVMBuildFPExt(self.llbuilder, val, dest_ty, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    fn ptrtoint(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe { 
            let instr = llvm::LLVMBuildPtrToInt(self.llbuilder, val, dest_ty, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    fn inttoptr(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe { 
            let instr = llvm::LLVMBuildIntToPtr(self.llbuilder, val, dest_ty, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    fn bitcast(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe { 
            let instr = llvm::LLVMBuildBitCast(self.llbuilder, val, dest_ty, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    fn pointercast(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe { 
            let instr = llvm::LLVMBuildPointerCast(self.llbuilder, val, dest_ty, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    fn icmp(&mut self, op: IntPredicate, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        ...
        unsafe { 
            let instr = llvm::LLVMBuildICmp(self.llbuilder, op as c_uint, lhs, rhs, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    fn fcmp(&mut self, op: RealPredicate, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        ...
        unsafe { 
            let instr = llvm::LLVMBuildFCmp(self.llbuilder, op as c_uint, lhs, rhs, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    fn select(
        ...
        unsafe { 
            let instr = llvm::LLVMBuildSelect(self.llbuilder, cond, then_val, else_val, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    fn va_arg(&mut self, list: &'ll Value, ty: &'ll Type) -> &'ll Value {
        unsafe { 
            let instr = llvm::LLVMBuildVAArg(self.llbuilder, list, ty, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    fn extract_element(&mut self, vec: &'ll Value, idx: &'ll Value) -> &'ll Value {
        unsafe { 
            let instr = llvm::LLVMBuildExtractElement(self.llbuilder, vec, idx, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    fn extract_value(&mut self, agg_val: &'ll Value, idx: u64) -> &'ll Value {
        assert_eq!(idx as c_uint as u64, idx);
        unsafe { 
            let instr = llvm::LLVMBuildExtractValue(self.llbuilder, agg_val, idx as c_uint, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    fn insert_value(&mut self, agg_val: &'ll Value, elt: &'ll Value, idx: u64) -> &'ll Value {
        assert_eq!(idx as c_uint as u64, idx);
        unsafe { let instr = llvm::LLVMBuildInsertValue(self.llbuilder, agg_val, elt, idx as c_uint, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    fn cleanup_ret(&mut self, funclet: &Funclet<'ll>, unwind: Option<&'ll BasicBlock>) {
        unsafe {
            let instr = llvm::LLVMRustBuildCleanupRet(self.llbuilder, funclet.cleanuppad(), unwind)
    ...
    fn catch_switch(
        ...
        if self.safety {
            self.insert_unsafe_metadata(ret);
        }
    ...
    fn call(
        ...
        unsafe {
            let instr = llvm::LLVMRustBuildCall(
                self.llbuilder,
                llty,
                llfn,
                args.as_ptr() as *const &llvm::Value,
...
impl<'a, 'll, 'tcx> Builder<'a, 'll, 'tcx> {
    fn with_cx(cx: &'a CodegenCx<'ll, 'tcx>) -> Self {
        ...
        Builder { llbuilder, cx, safety: false, }
    }
    ...
    // unsafe metadata insertion function - ydy
    ...
    pub fn insert_element(
        ...
        unsafe { 
            let instr = llvm::LLVMBuildInsertElement(self.llbuilder, vec, elt, idx, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    pub fn shuffle_vector(
        ...
        unsafe { 
            let instr = llvm::LLVMBuildShuffleVector(self.llbuilder, v1, v2, mask, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    pub fn vector_reduce_max(&mut self, src: &'ll Value, is_signed: bool) -> &'ll Value {
        unsafe { let instr = llvm::LLVMRustBuildVectorReduceMax(self.llbuilder, src, is_signed);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    pub fn va_arg(&mut self, list: &'ll Value, ty: &'ll Type) -> &'ll Value {
        unsafe { let instr = llvm::LLVMBuildVAArg(self.llbuilder, list, ty, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
    ...
    pub(crate) fn landing_pad(
        ...
        unsafe {
            let instr = llvm::LLVMBuildLandingPad(self.llbuilder, ty, None, num_clauses as c_uint, UNNAMED);
            if self.safety {
                self.insert_unsafe_metadata(instr);
            }
            instr
