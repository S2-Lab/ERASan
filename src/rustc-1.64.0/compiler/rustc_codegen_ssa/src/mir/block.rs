...
impl<'a, 'tcx> TerminatorCodegenHelper<'tcx> {
    ...
    fn do_call<Bx: BuilderMethods<'a, 'tcx>>(
        ...
        if let Some(unwind_block) = unwind_block {
            ...
            let invokeret: <Bx as BackendTypes>::Value =
...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    ...
    fn codegen_drop_terminator(
        ...
        let place_ty = location.ty(&self.mir.local_decls, self.cx.tcx()).ty;
        if place_ty.is_unsafe_ptr() {
            bx.rawptr_metadata(place.llval);
    ...
    fn codegen_call_terminator(
        ...
        // /* Check whether terminator requires unsafe or not */
        // let func_ty = func.ty(self.mir, bx.tcx());
        // // let func_id = 
        // //     if let ty::FnDef(func_id, _) = func_ty.kind() { Some(func_id) } else { None };
        // let sig = func_ty.fn_sig(bx.tcx());
        // if let hir::Unsafety::Unsafe = sig.unsafety() {
        //     match callee.val {
        //         OperandValue::Immediate(value) => {
        //             bx.unsafe_metadata(value);
        //         }
        //         OperandValue::Pair(value1,value2) => {
        //             bx.unsafe_metadata(value1);
        //             bx.unsafe_metadata(value2);
        //         }
        ...
        /* Check args whether raw ptr or not */
        for operand in args {
            match operand {
                mir::Operand::Copy(place) | mir::Operand::Move(place) =>{
                    let place_ty = place.ty(self.mir, bx.tcx()).ty;
                    let destination_ty = destination.ty(self.mir, bx.tcx()).ty;
                    if place_ty.is_unsafe_ptr() || destination_ty.is_unsafe_ptr() {
                        match callee.val {
                            OperandValue::Immediate(value) => {
        ...
        if intrinsic == Some(sym::caller_location) {
            if let Some(target) = target {
                let location = self.get_caller_location(&mut bx, mir::SourceInfo { span: fn_span, ..source_info });
                /* Check args whether raw ptr or not */
                for operand in args {
                    match operand {
                        mir::Operand::Copy(place) | mir::Operand::Move(place) =>{
        ...
        match intrinsic {
            ...
            Some(intrinsic) => {
                let dest: <Bx as BackendTypes>::Value = match ret_dest {
                ...
                let args: Vec<_> = args
                    ...
                    .map(|(i, arg)| {
                        ...
                        let operand = self.codegen_operand(&mut bx, arg);

                        /* Check Raw Pointer or not */
                        match arg {
                            mir::Operand::Copy(place) | mir::Operand::Move(place) =>{
                                let place_ty = place.ty(self.mir, bx.tcx()).ty;
                                let destination_ty = destination.ty(self.mir, bx.tcx()).ty;
        ...
        'make_args: for (i, arg) in first_args.iter().enumerate() {
            let mut op: OperandRef<'_, <Bx as BackendTypes>::Value> = self.codegen_operand(&mut bx, arg);
        ...
        let operands: Vec<_> = operands
            ...
            .map(|op| match *op {
                ...
                mir::InlineAsmOperand::Out { reg, late, ref place } => {
                    ...
                    // unsafety instrumentation
                    //  bx.unsafe_metadata(place.unwrap().llval);
                ...
                mir::InlineAsmOperand::InOut { reg, late, ref in_value, ref out_place } => {
                    ...
                    // match out_place {
                    //     Some(PlaceRef { llval,.. }) => {
                    //         bx.unsafe_metadata(llval)
                    //     },
                    //     None => {},
                    // }

                    // bx.unsafe_metadata(out_place.unwrap().llval);
...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    pub fn codegen_block(&mut self, bb: mir::BasicBlock) {
        ...
        let mut bx: Bx = Bx::build(self.cx, llbb);
        let mir = self.mir;
        let data = &mir[bb];
        ...
        for statement in &data.statements {
            ...
            // if let scope_data = mir.source_scopes[statement.source_info.scope].local_data
            //     .as_ref()
            //     .assert_crate_local() {
            //         if let mir::Safety::ExplicitUnsafe(_) = scope_data.safety {
            //             true;
            //         }
            //         else {
            //             false;
            ...
            // if let mir::Safety::ExplicitUnsafe(_) = scope_data {
            //     unsafe_or_not = true;
            // }
            // else if let mir::Safety::BuiltinUnsafe = scope_data {
            //     unsafe_or_not = true;   
            // }
            // else if let mir::Safety::FnUnsafe = scope_data {
            //     unsafe_or_not = true;
            // }
            // else {
            //     unsafe_or_not = false;
            ...
            // unsafe check - ydy
            // let mut file = OpenOptions::new().write(true).create(true).append(true).open("safety.txt").unwrap();
            let unsafe_or_not = if let Some(scope_data) = mir.source_scopes.get(statement.source_info.scope) {
            ...
            // writeln!(&mut file, "mir.source_scopes : {:?}", mir.source_scopes).expect("Unable to write file");
            // writeln!(&mut file, "statement.source_info : {:?}", statement.source_info ).expect("Unable to write file");
            // writeln!(&mut file, "statement.source_info.scope : {:?}", statement.source_info.scope ).expect("Unable to write file");
            // writeln!(&mut file, "unsafe_or_not : {:?}", unsafe_or_not ).expect("Unable to write file");
    ...
    fn codegen_terminator(
        ...
        // unsafety check
        let unsafe_or_not = if let Some(scope_data) = self.mir.source_scopes.get(terminator.source_info.scope) {
            if let mir::ClearCrossCrate::Set(ref localdata) = scope_data.local_data {
                if let mir::Safety::ExplicitUnsafe(_) = localdata.safety {
                    true
                } else if let mir::Safety::BuiltinUnsafe = localdata.safety {
